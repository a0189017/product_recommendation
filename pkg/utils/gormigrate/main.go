...
func (g *Gormigrate) migrate(migrationID string) error {
	...
	// init bar
	bar := pb.New(len(g.migrations))
	bar.Set("prefix", "[MIGRATION-PROGRESS] ")
	// the line break is necessary
...
`)
	...
	for _, migration := range g.migrations {
		bar.Set("prefix", fmt.Sprintf("[MIGRATION-PROGRESS][%s] ", migration.ID))
		bar.Write()
		if err := g.runMigration(migration); err != nil {
			bar.Set("prefix", fmt.Sprintf("[MIGRATION-FAILED][%s] ", migration.ID))
			bar.Write()
			return err
		}
		bar.Set("prefix", fmt.Sprintf("[MIGRATION-SUCCESS][%s] ", migration.ID))
		bar.Increment()
		bar.Write()
